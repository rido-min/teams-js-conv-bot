...
class TeamsConversationBot extends TeamsActivityHandler {
    constructor() {
        ...
        let membersAdded = [];
        this.onMembersAddedActivity(membersAdded, async (context, next) => {
        ...
        this.onTeamsReadReceiptEvent(async (readReceiptInfo, turnContext, next) => {
            let memberDetails = teamMemberDetails.find(member => member.aadObjectId === turnContext.activity.from.aadObjectId);
            if (memberDetails && readReceiptInfo.isMessageRead(memberDetails.messageId)) {
                users.push(memberDetails.name);
                counter++;
                teamMemberDetails = teamMemberDetails.filter(member => member.aadObjectId !== turnContext.activity.from.aadObjectId);
            }
        });
